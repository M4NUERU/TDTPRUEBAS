...
const Login = ({ onLoginSuccess }) => {
    ...
    useEffect(() => {
        if (pin.length === 4) {
            // handleSubmit(); // Could auto-submit if desired
        }
    }, [pin]);
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[var(--bg-main)] text-[var(--text-main)] overflow-hidden">
            {/* Logo */}
            <div className="w-24 h-24 bg-gradient-to-br from-blue-600 to-blue-700 rounded-[2.5rem] flex items-center justify-center mb-8 shadow-2xl shadow-blue-500/20 animate-in zoom-in-50 duration-500">
                <Package size={48} className="text-white drop-shadow-xl" />
            </div>

            <div className="text-center mb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <h1 className="text-3xl font-black mb-1 uppercase tracking-tighter italic">Todo<span className="text-blue-600">Tejidos</span></h1>
                ...
                {matchingUsers.length > 0 ? (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="text-center mb-6">
                            <p className="text-[10px] font-black uppercase text-blue-600 tracking-widest mb-1">Múltiples perfiles con este PIN</p>
                        ...
                        <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto no-scrollbar pb-4 pr-1">
                            {matchingUsers.map(user => (
                                <button
                                    ...
                                    className="w-full p-4 bg-[var(--bg-card)] border border-[var(--border-ui)] rounded-2xl flex flex-col items-center hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/10 transition-all"
                                >
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
                                <button
                                    ...
                                    className="h-16 flex items-center justify-center bg-[var(--bg-card)] border border-[var(--border-ui)] rounded-2xl text-xl font-black hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all active:scale-90"
                                >
                                    {num}
                                </button>
                            ))}
                            ...
                            <button
                                ...
                                className="h-16 flex items-center justify-center bg-blue-600 text-white rounded-2xl shadow-xl shadow-blue-500/20 hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-30 disabled:grayscale"
                            >
